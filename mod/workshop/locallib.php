...
class workshop_user_plan implements renderable {
    ...
    public function __construct(workshop $workshop, $userid) {
        ...
        if ($workshop->useexamples and has_capability('mod/workshop:manageexamples', $workshop->context, $userid)) {
            ...
            if ($examplescount > 0) {
                $countsql = "SELECT count(*) FROM {workshop_assessments} a, {workshop_submissions} s WHERE a.submissionid = s.id AND a.weight = 1 AND a.grade IS NOT NULL AND s.example = 1 AND s.workshopid = :workshopid";
