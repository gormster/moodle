...
class local_teameval_feedback_testcase extends advanced_testcase {
    ...
    public function test_all_feedback() {
        ...
        foreach ($feedbacks as $question) {
            $this->assertEquals("Mock feedback question", $question->title);
            $this->assertCount(5, $question->feedbacks);

            $first = current($question->feedbacks);
            $this->assertEquals(get_string('yourself', 'local_teameval'), $first->from);
        }
        ...
        $feedbacks = $this->teameval->all_feedback($studentN->id);
    ...
    public function test_empty_feedback() {
        ...
        $question = reset($feedbacks);

        $this->assertCount(4, $question->feedbacks);

        $first = current($question->feedbacks);
        $this->assertNotEquals(get_string('yourself', 'local_teameval'), $first->from);
    }
    ...
    public function test_rescind_feedback() {
        ...
        // TODO: Modify output\feedback to (a) use all_feedback and (b) have a way of getting
        // at its context object from a test environment
