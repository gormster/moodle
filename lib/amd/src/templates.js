...
define([ 'core/mustache',
     ...
     * @return {string}
     */
    var getJS = function(requiredJS, strings) {
        var js = '';
        ...
        loadPixTemplate.done(
            function() {
                ...
                if (requiredStrings.length > 0) {
                    // this object can be reset by a subsequent call to render()
                    var _requiredJS = requiredJS;
                    ...
                    str.get_strings(requiredStrings).done(
                        function(strings) {
                            ...
                            deferred.resolve(result.trim(), getJS(_requiredJS, strings));
                        }
                    ).fail(
                        function(ex) {
                            deferred.reject(ex);
                    ...
                    deferred.resolve(result.trim(), getJS(requiredJS, []));
                }
            }
        ).fail(
            function(ex) {
